# Shhh Be Quiet Function

`Formatting` `Language Fundamentals` `Strings`

[View on Edabit](https://edabit.com/challenge/wJBWjX5kSPMKLFmPX)

---

Write a function that removes all capital letters from a sentence except the first letter, puts quotation marks around the sentence and adds _", whispered Edabit."_ at the end.

### **Examples**

```
shhh("HI THERE!") ➞ '"Hi there!", whispered Edabit.'

shhh("tHaT'S Pretty awesOme") ➞ '"That's pretty awesome", whispered Edabit.'

shhh("") ➞ '"", whispered Edabit.'
```

### **Notes**

Don't forget to surround the original string with double quotation marks `""`.
